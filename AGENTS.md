...
Use the lookup index before broad file scanning:
...
This project uses [beads_rust](https://github.com/steveyegge/beads) for issue tracking. Issues are stored in `.beads/` and tracked in git.
...
br stats
br sync --flush-only  # Export/update .beads JSONL snapshots (no git commit)
git add .beads/
...
br sync --flush-only    # Export beads updates
git add .beads/
git commit -m "sync beads"
